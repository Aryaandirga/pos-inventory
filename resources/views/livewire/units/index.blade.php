<div>
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
        <div>
            <h2 class="text-2xl font-black text-gray-900 tracking-tight">Satuan Produk</h2>
            <p class="text-sm text-gray-400 mt-1 font-medium">Kelola metrik takaran (pcs, kg, liter, dll)</p>
        </div>
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-gray-50/50">
                        ...
                        <th class="px-6 py-4 text-xs font-black text-gray-400 uppercase tracking-widest">Nama Satuan</th>
                        <th class="px-6 py-4 text-xs font-black text-gray-400 uppercase tracking-widest">Singkatan</th>
                        <th class="px-6 py-4 text-xs font-black text-gray-400 uppercase tracking-widest text-center">Penggunaan</th>
                    ...
                    <tr class="hover:bg-indigo-50/30 transition-all group">
                        <td class="px-6 py-5">
                            <span class="text-xs font-black text-gray-300 group-hover:text-indigo-300 transition-colors">
                                {{ str_pad($units->firstItem() + $loop->index, 2, '0', STR_PAD_LEFT) }}
                            </span>
                        </td>
                        <td class="px-6 py-5 text-sm font-bold text-gray-800 group-hover:text-indigo-600 transition-colors">
                            {{ $unit->name }}
                        </td>
                        <td class="px-6 py-5">
                            <span class="inline-flex items-center px-3 py-1 bg-gray-100 text-gray-600 text-[10px] font-black uppercase tracking-wider rounded-lg group-hover:bg-white transition-colors">
                                {{ $unit->abbreviation }}
                            </span>
                        </td>
                        <td class="px-6 py-5 text-center">
                            <span class="inline-flex items-center px-3 py-1 bg-indigo-50 text-indigo-600 text-xs font-bold rounded-full group-hover:bg-indigo-100 transition-colors">
                                {{ $unit->products->count() }} Produk
                            </span>
                        </td>
                        <td class="px-6 py-5">
                        ...
                        <td colspan="5" class="px-6 py-20">
                            <div class="flex flex-col items-center justify-center text-center">
                                <div class="w-20 h-20 bg-gray-50 rounded-[2rem] flex items-center justify-center mb-4 border border-dashed border-gray-200">
                                    <svg class="w-10 h-10 text-gray-200" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="1.5"><path d="M3 6l3 1m0 0l-3 9a5.002 5.002 0 006.001 0M6 7l3 9M6 7l6-2m6 2l3-1m-3 1l-3 9a5.002 5.002 0 006.001 0M18 7l3 9m-3-9l-6-2m0-2v2m0 16V5m0 16H9m3 0h3"/></svg>
                                </div>
                                <h3 class="text-gray-900 font-black text-lg">Belum Ada Satuan</h3>
                                <p class="text-gray-400 text-sm max-w-[240px] mt-1 font-medium">Buat satuan baru untuk mulai mendefinisikan stok produk Anda.</p>
                                <a href="{{ route('units.create') }}" class="mt-6 text-indigo-600 font-bold text-sm hover:text-indigo-700 underline underline-offset-4">Tambah Satuan Sekarang</a>
                            </div>
                        </td>
                    </tr>
                    @endforelse
    ...
    <div class="fixed inset-0 z-[60] flex items-center justify-center p-4">
        ...
        <div class="bg-white rounded-[2.5rem] p-8 w-full max-w-sm shadow-2xl relative z-10 animate-scale-up">
            ...
            <h3 class="text-xl font-black text-gray-900 text-center mb-2 tracking-tight">Hapus Satuan?</h3>
            <p class="text-sm text-gray-400 text-center mb-8 font-medium px-4">Satuan yang dihapus tidak dapat dipulihkan. Pastikan tidak ada produk yang menggunakan satuan ini.</p>
